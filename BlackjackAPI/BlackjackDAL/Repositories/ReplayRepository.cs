	...
	public class ReplayRepository : IReplayRepository
	{
		...
		public async Task<List<ReplayModel>> RetrieveGameReplayAsync(List<int> rounds, string group_id)
		{
			try
			{
				using (var context = new AppDbContext(_DBConnection.ConnectionString()))
				{
					var gameActions = context.History
			...
			try
			{
				using (var context = new AppDbContext(_DBConnection.ConnectionString()))
				{
					...
					foreach (var round in rounds)
					{
						...
						var roundTimestamps = await context.History
						...
						var roundMessages = await (from m in context.Message
		...
		public async Task<List<int>> RetrieveGameRoundsAsync(int user_id, string group_id)
		{
			try
			{
				using (var context = new AppDbContext(_DBConnection.ConnectionString()))
				{
					var rounds = await context.History
